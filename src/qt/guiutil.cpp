...
{
    // Convert impleum:// to impleum:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("impleum://"))
    {
        uri.replace(0, 12, "impleum:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Impleum.lnk";
}
...
{
    return GetAutostartDir() / "impleum.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Impleum-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  impleum-qt [" + tr("command-line options") + "]                     " + "\n";
